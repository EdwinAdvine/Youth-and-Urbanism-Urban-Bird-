...
export default function CheckoutPage() {
  ...
  const handleApplyCoupon = async () => {
    try {
      const res = await api.post("/api/v1/coupons/validate", { code: couponInput, order_subtotal: subtotal });
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (paymentMethod === "paystack") {
        ...
        const paystack = await orderService.initializePaystack(order.id);
        resetCheckout();
  ...
  return (
    <div className="container-custom py-6 sm:py-8 max-w-4xl">
      <h1 className="text-2xl sm:text-3xl font-bold font-lexend text-gray-900 mb-6 sm:mb-8">Checkout</h1>

      {/* Sign-in prompt for guests */}
      {!user && (
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 p-4 bg-gray-50 border border-gray-200 rounded-xl">
          <div className="flex items-center gap-3">
            <LogIn size={20} className="text-maroon-700 flex-shrink-0" />
            <div>
              <p className="text-sm font-semibold font-manrope text-gray-900">Already have an account?</p>
              <p className="text-xs text-gray-500 font-manrope">Sign in to auto-fill your details and track orders easily.</p>
            </div>
          </div>
          <div className="flex gap-2 flex-shrink-0">
            <Link to={`/account/login?next=/checkout`}>
              <button className="px-4 py-2 text-sm font-semibold font-manrope text-white bg-maroon-700 hover:bg-maroon-800 rounded-lg transition-colors">
                Sign In
              </button>
            </Link>
            <div className="flex items-center gap-1 px-3 py-2 text-sm text-gray-500 font-manrope">
              <UserX size={14} />
              <span>Continue as guest below</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          ...
          {step === "shipping" && (
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <Input label="First Name" name="first_name" defaultValue={shippingData?.first_name || user?.first_name || ""} required />
        ...
        <div className="md:col-span-1">
          <div className="bg-white rounded-xl border border-gray-100 p-4 sm:p-5 sticky top-20 md:top-24">
              ...
              {cart?.items?.map((item) => (
                <div key={item.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-manrope font-medium text-gray-900 truncate">{item.product_name ?? item.variant?.product?.name}</p>
                    <p className="text-xs text-gray-500 font-manrope">{item.size ?? item.variant?.size} · ×{item.quantity}</p>
                  </div>
